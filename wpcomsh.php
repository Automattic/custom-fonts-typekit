...
/**
 * if not using WordPress.com Nameservers and site pointed to Pressable IPs
 * show an error notice that they need to update to new IPs or their
 * site stops working
 */
function wpcomsh_no_wpcom_nameserver_pressable_a_record() {
	// If we're not on Pressable, skip
	...
	$nameservers_response = dns_get_record( $domain, DNS_NS );
	$a_records_response = dns_get_record( $domain, DNS_A );

	if ( false === $nameservers_response || false === $a_records_response ) {
		return;
	}

	$nameservers = wp_list_pluck( $nameservers_response, 'target' );
	$a_records = wp_list_pluck( $a_records_response, 'ip' );

	$wpcom_nameservers = array_reduce(
		$nameservers,
		function( $reduced_value, $item ) {
			$item = strtolower( $item );

			return $reduced_value && in_array( $item, [ 'ns1.wordpress.com', 'ns2.wordpress.com', 'ns3.wordpress.com' ] );
		},
		true
	);

	$pressable_ips = array_reduce(
		$a_records,
		function( $reduced_value, $item ) {
			return $reduced_value || strpos( $item, '209.15.20.') === 0 || strpos( $item, '66.155.38.') === 0;
		},
		false
	);

	if ( ! $wpcom_nameservers && $pressable_ips ) {
		$atomic_ips = get_atomic_ip_addresses( $domain );

		$ip_list = '';
		foreach ( $atomic_ips as $atomic_ip ) {
			$ip_list .= "<li><strong>$atomic_ip</strong></li>";
		}

		echo '
		<div class="notice notice-error is-dismissible">
			<p>
				Your site currently uses <em>A records</em> which will soon be <strong>deprecated</strong>. To keep your site running without any downtime, please update your <em>A records</em> to:
			</p>
			<ul>' . $ip_list . '</ul>
			<p>
				If you have trouble updating the records, feel free to <a target="_blank" href="https://wordpress.com/help/contact">contact us</a>.
			</p>
		</div>
		';
	}
